...
<div class="admin-dashboard">
    <div class="admin-header">
        ...
        <p class="lead" style="color: var(--body-muted); font-size: 18px;">Manage users, assets, and conversion tasks.</p>
    </div>
        ...
        <div class="table-header">
            ...
            <div class="table-filters">
                <form action="{{ route('admin.dashboard') }}" method="GET" class="filter-form">
                    <select name="status" class="form-select" style="width: auto; padding: 8px 12px; font-size: 14px;" onchange="this.form.submit()">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <div class="table-thumb">
                            @if($asset->thumbnail_path)
                                <img src="{{ asset('storage/' . $asset->thumbnail_path) }}" alt="Thumb">
                            @else
                    ...
                    <td style="font-weight: 500; color: var(--on-dark);">
                        <a href="{{ route('assets.show', $asset) }}" target="_blank" class="asset-link">{{ Str::limit($asset->title, 30) }}</a>
                    </td>
                    <td>{{ $asset->user->name ?? 'Unknown' }}</td>
                    <td>
                        <span class="status-badge status-{{ $asset->status->value }}">{{ ucfirst($asset->status->value) }}</span>
                    </td>
                    <td>
                    ...
                    <td>
                        <div style="display: flex; gap: 8px; align-items: center;">
                            <form action="{{ route('admin.assets.toggle-visibility', $asset) }}" method="POST" style="margin: 0;">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="btn-action" style="white-space: nowrap; padding: 6px 12px; color: {{ $asset->visibility->value === 'public' ? '#f59e0b' : '#10b981' }}; border-color: rgba(255,255,255,0.1); background: rgba(255,255,255,0.05);" title="Toggle Visibility">
                                    {{ $asset->visibility->value === 'public' ? 'Make Private' : 'Make Public' }}
                                </button>
                            </form>
                            <form action="{{ route('admin.assets.destroy', $asset) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this asset? This cannot be undone.');" style="margin: 0;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn-action" style="white-space: nowrap; padding: 6px 12px; color: #ef4444; border-color: rgba(239, 68, 68, 0.2); background: rgba(239, 68, 68, 0.05);" title="Permanently Delete">Delete</button>
                            </form>
                        </div>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="7" style="text-align: center; padding: 40px; color: var(--body-muted);">No assets found in the system.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
